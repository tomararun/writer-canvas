    ...
    <article>
      <header className="border-b border-rule bg-paper">
        <div className="wrap py-16 md:py-24">
          <nav aria-label="Breadcrumb" className="text-xs text-muted-foreground">
            <Link to="/case-studies" className="link-underline">Case studies</Link>
      ...
      <div className="wrap grid gap-16 py-16 md:py-24">
        <section aria-labelledby="background" className="grid gap-6 md:grid-cols-[1fr_2fr]">
          <h2 id="background" className="font-display text-2xl">Background</h2>
        ...
        <section aria-labelledby="process" className="grid gap-6 md:grid-cols-[1fr_2fr]">
          <h2 id="process" className="font-display text-2xl">Process</h2>
          <ol className="grid gap-6">
            {study.process.map((s: { step: string; detail: string }, i: number) => (
              <li key={s.step} className="grid grid-cols-[2rem_1fr] gap-4 border-b border-rule pb-6">
        ...
        <section aria-labelledby="implementation" className="grid gap-6 md:grid-cols-[1fr_2fr]">
          <h2 id="implementation" className="font-display text-2xl">Implementation</h2>
          <ul className="grid gap-4">
            {study.implementation.map((s: string) => (
              <li key={s} className="border-l-2 border-primary pl-4 leading-relaxed">{s}</li>
            ))}
          </ul>
        </section>

        <section aria-labelledby="gallery">
          <h2 id="gallery" className="font-display text-2xl">Gallery</h2>
        ...
        <section aria-labelledby="outcomes" className="border-y border-rule bg-paper p-8 md:p-12">
          <h2 id="outcomes" className="font-display text-2xl">Outcomes</h2>
        ...
        <section aria-labelledby="learnings" className="grid gap-6 md:grid-cols-[1fr_2fr]">
          <h2 id="learnings" className="font-display text-2xl">What I learned</h2>
          <ul className="grid gap-4">
            {study.learnings.map((l: string) => (
              <li key={l} className="border-b border-rule pb-4 text-lg leading-relaxed">{l}</li>
            ))}
          </ul>
        </section>

        <p>
          <Link to="/case-studies" className="link-underline text-sm">← All case studies</Link>
        </p>
      </div>
    </article>
  );
